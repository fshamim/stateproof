 ...
 * against the iCages MainStateMachineTest.kt test cases.
 *
 * This proves that StateProof generates the same (or better) test coverage.
 */
class ICagesValidationTest {

    /**
     * Recreates the iCages main state machine info structure.
     * This is equivalent to getMainStateMachineInfo() in iCages.
     */
    private fun getICagesStateMachineInfo(): Map<String, StateInfo> {
        ...
        // Build state machine info (same as iCages getMainStateMachineInfo)
    ...
    /**
     * Expected transitions from iCages MainStateMachineTest.kt
     * These are the actual test cases that exist in iCages.
     */
    private val iCagesExpectedTransitions = listOf(
        // From MainStateMachineTest.kt - all expectedTransitions lists
    ...
    @Test
    fun stateProof_shouldGenerateSamePathsAsICages() {
        val stateInfo = getICagesStateMachineInfo()
        ...
        println("iCages has ${iCagesExpectedTransitions.size} test cases")

        // Check that all iCages test cases are covered
        var allCovered = true
        for (expected in iCagesExpectedTransitions) {
        ...
        assertTrue(allCovered, "All iCages test cases should be covered by StateProof")

        // StateProof may generate MORE paths (which is fine - better coverage)
        println("\nStateProof generates ${generatedPaths.size} paths vs iCages ${iCagesExpectedTransitions.size} tests")

        // Show any additional paths StateProof generates
        val additionalPaths = generatedTransitions - iCagesExpectedTransitions.toSet()
    ...
    fun stateProof_shouldGenerateCorrectTestNames() {
        ...
        val enumerator = SimplePathEnumerator(
            stateInfoMap = stateInfo,
            initialState = "Initial",
            config = TestGenConfig(hashAlgorithm = TestGenConfig.HashAlgorithm.CRC16),  // Use CRC16 like iCages
        )
    ...
    @Test
    fun stateProof_shouldMatchICagesPathCount() {
        val stateInfo = getICagesStateMachineInfo()
        val enumerator = SimplePathEnumerator(
            stateInfoMap = stateInfo,
            initialState = "Initial",
        ...
        // StateProof should generate at least as many paths as iCages
        assertTrue(
            generatedPaths.size >= iCagesExpectedTransitions.size,
            "StateProof should generate at least ${iCagesExpectedTransitions.size} paths, got ${generatedPaths.size}"
        )
    }
